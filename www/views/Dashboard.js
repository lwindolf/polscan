...
Dashboard.prototype.addTopChanges = function(changes, type) {
	var results = [];
        var sortbar = [];
...
Dashboard.prototype.update = function(params) {
	...
	$("#filter").hide();
	$("<div id='DashboardBoxContainer'>").appendTo(this.parentDiv);
	$("<div id='col2'>").appendTo("#DashboardBoxContainer");
	$("<div id='col1'>").appendTo("#DashboardBoxContainer");

	getData("overview", function(data) {
			$("#row1").append("<div class='chart'><span id='DashboardCalendar'/></div>");
			addCalendar("#DashboardCalendar", data.date);
			$("#loadmessage").hide();
